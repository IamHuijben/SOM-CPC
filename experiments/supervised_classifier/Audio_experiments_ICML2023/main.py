...
if __name__ == "__main__":
    ...
    # Run a classifier on top of a trained CPC model #
    runs, run_names = [],[]

    # Example checkpoint_path, fill in your own:
    checkpoint_path = Path.cwd() / "CPC" / "Audio_experiments_ICML2023" / "CPC_audio" / "checkpoints" / "model_2269.pt"
    runs.append({
        'model.encoder.checkpoint_path': Path(checkpoint_path),
        'model.ARmodel.checkpoint_path': Path(checkpoint_path),
    })
    run_names.append(f'')
    ...
    for run_name_comment, run in zip(run_names, runs):
        ...
        save_dir = set_save_dir_continue_training(Path(run['model.encoder.checkpoint_path']).parent.parent, f'supervised_clas_e{epoch}{run_name_comment}')
